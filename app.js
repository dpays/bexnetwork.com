...
let defaultWebsocket = 'https://dpayapi.com';

let getBlockchainVersion = function() {
	dpay.api.getConfig(function(err, result) {
		console.log(result.DPAY_BLOCKCHAIN_VERSION);
		if ( ! err) document.getElementById('blockchain-version').innerHTML = result.DPAY_BLOCKCHAIN_VERSION;
	});
};

let getChainProperties = function() {
	dpay.api.getChainProperties(function(err, properties) {
		if ( ! err) {
...
let nodeAddress = dpay.api.setOptions({ url: 'https://dpayapi.com' });
...
dpay.api.streamBlockNumber(function(err, lastBlock) {
	if ( ! err) {
		dpay.api.getBlock(lastBlock, function(err, block) {
	...
	dpay.api.getDynamicGlobalProperties(function(err, properties) {
		if ( ! err) {
			for (let key in properties) {
...
let getBlockFullInfo = function(blockNumberVal) {
	...
	dpay.api.getBlock(blockNumberVal, function(err, block) {
		loadingHide();
		if (block) {
...
document.getElementById('search-hex').addEventListener('submit', function(e) {
	...
	$resetAccountBtn.style.display = 'none';
	$recentBlocksInfo.style.display = 'none';
	dpay.api.getTransaction(hexNumberVal, function(err, result) {
		loadingHide();
		if ( ! err) {
...
let getAccountTransactions = function() {
	...
	dpay.api.getAccountHistory(usernameVal, accountHistoryFrom, accountHistoryCount, function(err, transactions) {
		loadingHide();
